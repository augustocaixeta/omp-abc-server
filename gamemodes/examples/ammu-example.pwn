...
public OnPlayerCommandText(playerid, cmdtext[]) {
    ...
    if (strequal(cmdtext, "/menu")) {
        for (new i; i != 15; ++i) {
            AddListMenuItemStr(playerid, 0, str_format("Item: %i", i));
            AddListMenuItem(playerid, 1, "");
        }

        AddListMenuItemSelector(playerid, 2, "Option A");
        AddListMenuItemSelector(playerid, 2, "Option B");
        AddListMenuItemSelector(playerid, 2, "Option C");
        
        AddListMenuItemSelector(playerid, 9, "Far Option 1");
        AddListMenuItemSelector(playerid, 9, "Far Option 2");

        new const
            Task:t = ShowAsyncListMenu(playerid, "Menu", 20.0, 120.0, 250.0)
        ;
        ...
        if (t) {
            ...
            SendClientMessage(playerid, -1, "Item %i selected.", responses[E_ASYNC_MENU_LISTITEM]);
        }
